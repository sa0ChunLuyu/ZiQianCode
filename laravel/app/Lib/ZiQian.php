...
$data_str");
  ...
  public static function route($type = 'map')
  {
    $path_array = [
      '/Http/Controllers'
    ];
    ...
    foreach ($path_array as $path) {
      ...
      if ($type == 'include') {
        if (isset($_SERVER['REQUEST_URI'])) {
          ...
          if (count($uri_array) >= 5) {
            $contorller_path = "$dir_path/$uri_array[3]Controller.php";
            if (is_file($contorller_path)) {
              $pick_function = $uri_array[4];
              $controller_files[] = "$uri_array[3]Controller.php";
            }
          }
        }
      } else {
      ...
      if (preg_match($class_pattern, $file_content, $class_matches)) {
        ...
        if (count($blocks) != 0) {
          foreach ($blocks as $block) {
            ...
            if (!!$pick_function && $route_name != $pick_function) {
              continue;
            }
