...
const checkLastTimeIp = () => {
  if (!$ip_notification.value) {
    ...
    window
      .$notification()
      [
        $store.info.last_login.ip !== $store.info.previous_login.ip
          ? "warning"
          : "success"
      ]({
        title: "登录信息",
        ...
        message:
          `<div>
<b>本次登录地址:</b> ${$store.info.last_login.ip}<br>
<b>本次登录信息:</b> ${$store.info.last_login.region}<br>
<b>本次登录时间:</b> ${$store.info.last_login.created_at}
</div>
` +
          ($store.info.last_login.ip !== $store.info.previous_login.ip
            ? `<hr>
<div>
<b>上次登录地址:</b> ${$store.info.previous_login.ip}<br>
<b>上次登录信息:</b> ${$store.info.previous_login.region}<br>
<b>上次登录时间:</b> ${$store.info.previous_login.created_at}
</div>`
            : ""),
      });
  }
...
const AdminAccountChangePassword = async () => {
  if (
    change_password_data.value.password !==
    change_password_data.value.check_password
  ) {
...
const updateSelfClick = () => {
  update_self_data.value = JSON.parse(
    JSON.stringify({
      ...
      <div>
        <el-form label-width="70">
          ...
          <el-form-item label="昵称">
            <el-input
              v-model="update_self_data.nickname"
              placeholder="请输入昵称"
            ></el-input>
          </el-form-item>
        </el-form>
    ...
    >
        ...
        <div :class="[$store.info.initial_password === 1 ? 'mt-2' : '']">
          <el-form label-width="70">
            ...
            <el-form-item label="新密码">
              <el-input
                v-model="change_password_data.password"
                placeholder="请输入新密码"
                type="password"
              ></el-input>
            </el-form-item>
            <el-form-item label="确认密码">
              <el-input
                v-model="change_password_data.check_password"
                placeholder="请确认密码"
                type="password"
              ></el-input>
            </el-form-item>
            <el-form-item label="验证码">
              <div w-full>
                <el-input
                  v-model="change_password_data.code"
                  placeholder="请输入验证码"
                ></el-input>
                <div
                  v-loading="captcha_loading && !change_password_data.image"
                ...
                >
                  <div
                    w-full
                    h-full
                    v-if="!!change_password_data.image"
                    :style="{
                      backgroundImage:
                        'url(' + $image(change_password_data.image) + ')',
                    }"
                  ></div>
                </div>
              </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button @click="AdminAccountChangePassword()" type="primary"
            >保存</el-button
          >
        </div>
      </template>
